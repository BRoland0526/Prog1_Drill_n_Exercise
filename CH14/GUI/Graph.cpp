...
Circle_face::Circle_face(Point p, int rr)
    ...
    mouth(p, rr*0.35, rr*0.35, 180, 360)
{
...
void Circle_face::set_radius(int rr)
{
    ...
    mouth.set_minor(rr*0.35);
    mouth.set_major(rr*0.35);
}
...
// Frowny
Circle_face2::Circle_face2(Point p, int rr)
    : Circle(p,rr),
    l_eye(Point(p.x-rr/5, p.y-rr/2), rr/8, rr/3),
    r_eye(Point(p.x+rr/5, p.y-rr/2), rr/8, rr/3),
    mouth(p, rr*0.22, rr*0.22, 0, 180)
{
    l_eye.set_color(Color::invisible);
    l_eye.set_fill_color(color());
    r_eye.set_color(Color::invisible);
...
void Circle_face2::draw_lines() const
{
    Circle::draw_lines();
    if(color().visibility())
    {
...
void Circle_face2::set_color(Color c)
{
    Shape::set_color(c);
    l_eye.set_fill_color(c);
    r_eye.set_fill_color(c);
...
void Circle_face2::set_style(Line_style ls)
{
    Circle::set_style(ls);
    mouth.set_style(ls);
}

void Circle_face2::move(int dx, int dy)
{
    Shape::move(dx,dy);
    l_eye.move(dx,dy);
    r_eye.move(dx,dy);
...
void Circle_face2::set_radius(int rr)
{
    Circle::set_radius(rr);
    l_eye.set_minor(rr/3);
    l_eye.set_major(rr/8);
    ...
    mouth.set_minor(rr*0.30);
    mouth.set_major(rr*0.30);
}

Frowny::Frowny(Point p, int rr) : Circle_face2(p, rr) {}
